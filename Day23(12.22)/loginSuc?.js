...
function ExceptionOfNums(id_pw, db){
    if(id_pw.length != 2){
        throw new Error("id_pw 길이 오류");
    }
    if(db.length < 1 || db.length > 10){
        throw new Error("db 길이 오류");
    }
    for(let i = 0; i < db.length; i++){
        if(db[i].length != 2){
            throw new Error("db 원소 길이 오류");
        }
    }
}
